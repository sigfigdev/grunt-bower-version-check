        ...
        (function (bowerVersionCheck) {
            ...
            var BowerVersionCheck = (function () {
                    ...
                    this.getBowerList().then(function (list) {
                        ...
                        _.each(list.dependencies, function (dependency) {
                            ...
                            if (dependency.incompatible) {
                                _this.grunt.log.error("Incompatible dependency" + (dependency.linked ? " (linked)" : "") + ": " + dependency.pkgMeta.name + ". " + "Required " + dependency.endpoint.target + " but found " + dependency.pkgMeta.version);
                                failed = true;
                            }
                            ...
                            if (options.attemptHashVersionIncompatibilityDetection && BowerVersionCheck.isHashVersioned(dependency)) {
                                ...
                                if (dependency.endpoint.target !== actualHashVersion) {
                                    _this.grunt.log.error("Incompatible dependency" + (dependency.linked ? " (linked)" : "") + ": " + dependency.pkgMeta.name + ". " + "Required " + dependency.endpoint.target + " but found " + actualHashVersion);
                                    failed = true;
                                }
                            }
                        });
                ...
                BowerVersionCheck.isHashVersioned = function (dependency) {
                    return dependency.endpoint.target.length === 40 && dependency.endpoint.source.substr(-4) === ".git";
                };

                BowerVersionCheck.getActualHashVersion = function (dependency) {
                    return (dependency.pkgMeta._resolution && dependency.pkgMeta._resolution.commit) || undefined;
                };
